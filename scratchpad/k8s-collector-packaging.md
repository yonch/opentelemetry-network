...
- New image: `k8s-collector` packaged similarly to other agents. The Dockerfile is in `collector/Dockerfile.k8s-collector`, referenced from the k8s-collector CMake via `DOCKERFILE_NAME`.
...
- Build and packaging rules for the k8s collector live in `collector/CMakeLists.txt`.
...
- Docker: `k8s-collector-docker` and `k8s-collector-docker-registry` via `build_custom_docker_image()` using `collector/Dockerfile.k8s-collector`; we copy the stripped binary, `debug-info.conf`, and LICENSE/NOTICE.
...
- Added container packaging for the Rust collector:
  - Consolidated build rules into `collector/CMakeLists.txt`; no nested k8s-specific CMakeLists.
  - CMake target `k8s-collector` (via `add_rust_main`) and Docker packaging (`k8s-collector-docker` and `k8s-collector-docker-registry`). Dockerfile lives in `collector/Dockerfile.k8s-collector`.
  - Hooked `k8s-collector` into `collectors` and `collectors-docker` via CMake dependencies.
  - Removed the old `collector/k8s` directory (Go watcher and C++ relay); only the Rust `k8s-collector` is built and packaged now.
- CI:
