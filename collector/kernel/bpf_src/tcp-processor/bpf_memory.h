/*
 * Copyright The OpenTelemetry Authors
 * SPDX-License-Identifier: Apache-2.0
 */

//
// bpf_memory.h - BPF memory related utility functions
//

#pragma once

#include "bpf_debug.h"
#include "bpf_types.h"

// s2 cannot be longer than 16 bytes due to older BPF inlining/loop limits.
// Pass the length of s2 explicitly to avoid reading from program .rodata with
// helpers that expect kernel pointers (verifier-unsafe).
static __always_inline int string_starts_with(const char *s1, const size_t s1_len, const char *s2, const int s2_len)
{
  // Caller must ensure s1 points to stack-local memory (or otherwise safe
  // for direct access). For kernel/user pointers, copy to stack first.
  if (s2_len <= 0) {
    return 1;
  }
  if (s2_len > 16) {
    return 0;
  }
  if (s1_len < (size_t)s2_len) {
    return 0;
  }

  for (int i = 0; i < 16; i++) {
    if (i >= s2_len) {
      break;
    }
    if (s1[i] != s2[i]) {
      return 0;
    }
  }

  return 1;
}

static __always_inline int char_to_number(char x)
{
  if (x < '0' || x > '9')
    return -1;
  return (int)(x - '0');
}
